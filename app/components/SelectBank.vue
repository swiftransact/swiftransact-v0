...
<div>
    <div class="input-container relative">
      <div @click="showBankList = !showBankList" class="flex items-center gap-2.5 p-3">
          <span class="truncate max-w-[90px]">{{ modelValue?.bankName || 'Choose a Bank' }}</span>
          <icon :name="showBankList ? 'arrow_head_up' : 'arrow_head_down'" :size="16" color="var(--color-black)" />
      </div>
      <input
          type="text"
          inputmode="numeric"
    ...
    <p v-if="error || errorMessage" class="text-error text-xs mt-1">{{ error || errorMessage }}</p>
    <p v-else-if="modelValue?.accountName" class="text-xs text-success mt-1">{{ modelValue.accountName }}</p>
  </div>
</template>
...
const banks = computed(() => data.value?.data.filter((bank: Bank) => bank.name.toLowerCase().includes(searchQuery.value.toLowerCase())) as Bank[] || [])
...
/********************* useQuery *********************/
const { data } = useBank()
const { data: verificationData, isLoading: isVerifying, error: verificationError } = verifyAccount(accountNumber, bankCode)
...
/********************* Watchers *********************/
watch(() => verificationData.value, (newVal) => {
...
watch(() => verificationError.value, (error) => {
