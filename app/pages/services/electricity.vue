<template>
    <div class="flex flex-col items-center mt-[20%]">
        <select-prepaid class="w-fit" v-model="selectedProvider" />
         <service-input label="Meter Number" v-model="meterNumber" placeholder="Meter number" />
         <amount-input :onClick="handleConfirmPayment" class="mt-8" :disabled="!meterNumber || !amount"  v-model="amount" placeholder="Enter Amount" buttonTitle="Confirm & Pay" />
    </div>
  ...
  const modal: ComputedRef<Modal | null> = computed(() => {
    ...
    if(modalState.value === 'success') {
        return {
            ...
            primaryAction: () => {
                router.replace(`/receipts/electricity`)
